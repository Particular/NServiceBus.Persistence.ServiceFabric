...
* In the "NServiceBus.Persistence.ServiceFabric" solution, go to the "TestHarness" project and open the class representing the test suite you want to run (e.g. for the acceptance tests, open the `AcceptanceTests.cs` file)
* Click run tests in the opened file. It may take a while to connect to the SF cluster but once running, all executed tests will be shown in the test runner.
