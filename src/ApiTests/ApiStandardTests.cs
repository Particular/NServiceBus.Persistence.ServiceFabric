namespace ApiTests
{
    ...
    public class ApiStandardTests
    {
        ...
        public void NonPublicShouldHaveSimpleNamespace()
        {
            ...
            var types = typeof(ServiceFabricPersistence).Assembly.GetTypes()
                .Where(x =>
                    !x.IsPublic &&
                    !x.IsNested &&
        ...
        public void LoggersShouldBeStaticField()
        {
            ...
            foreach (var type in typeof(ServiceFabricPersistence).Assembly.GetTypes().Except(exceptions).Where(t => !exceptions.Contains(t.DeclaringType)))
            {
        ...
        {
            return typeof(ServiceFabricPersistence).Assembly.GetTypes()
                .Where(type => type.GetInterfaces().Any(face => face.Name == nameof(IBehavior)) && !type.IsAbstract && !type.IsGenericType);
        }
