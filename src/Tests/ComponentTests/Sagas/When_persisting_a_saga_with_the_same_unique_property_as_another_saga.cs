    ...
    public class When_persisting_a_saga_with_the_same_unique_property_as_another_saga : SagaPersisterTests
    {
        ...
        public async Task It_should_enforce_uniqueness()
        {
            ...
            using (var losingSession = await configuration.SynchronizedStorage.OpenSession(losingContextBag))
            {
                ...
                Assert.That(async () => await persister.Save(saga2, correlationPropertySaga2, losingSession, losingContextBag), Throws.InstanceOf<Exception>().And.Message.EndsWith($"The saga with the correlation id 'Name: {nameof(SagaWithCorrelationPropertyData.CorrelatedProperty)} Value: {correlationPropertyData}' already exists."));
            }
        }
    }
}